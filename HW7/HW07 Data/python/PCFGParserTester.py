...
class PCFGParser(Parser):
    ...
    def get_best_parse(self, sentence):
        ...
        self.score_in_grammar = collections.defaultdict(lambda : collections.defaultdict(lambda : collections.defaultdict(lambda : 0.0)))
        self.background_parse = collections.defaultdict(lambda : collections.defaultdict(lambda : collections.defaultdict(lambda : list())))

        #get the preterminal tag
        ...
        for level in range(1, len(sentence)):
            for begin in range(0, len(sentence) - level):
                ...
                for split_spot in range(begin + 1 , end):
                    self.handle_binary(begin, split_spot, end)
    ...
    def get_base_tags(self, sentence):
        for i in range(0, len(sentence)):
            for tag in self.lexicon.get_all_tags():
                score = self.lexicon.score_tagging(sentence[i], tag)
                if (score > 0):
                    self.score_in_grammar[0 + i][1 + i][tag] = score
    ...
    def handle_binary(self, begin, split, end):
        left_tags = self.score_in_grammar[begin][split].keys()
        for left_tag in left_tags:
            if left_tag in self.grammar.binary_rules_by_left_child:
                for binary_rule in self.grammar.binary_rules_by_left_child[left_tag]:
                    if binary_rule.right_child in self.score_in_grammar[split][end].keys():
                        score = self.score_in_grammar[begin][split][binary_rule.left_child] * \
                            self.score_in_grammar[split][end][binary_rule.right_child] * \
                            binary_rule.score
                        if not binary_rule.parent in self.score_in_grammar[begin][end] or score > self.score_in_grammar[begin][end][binary_rule.parent]:
                            self.score_in_grammar[begin][end][binary_rule.parent] = score
                            self.background_parse[begin][end][binary_rule.parent] = (split, binary_rule.left_child, binary_rule.right_child)

    def build_tree(self, begin, end,  sentence, best_tag = ""):
        best_tag = ""
        score = 0.0
        for item in self.score_in_grammar[begin][end].items():
            if item[1] > score:
                score = item[1]
                best_tag = item[0]
        parse = self.background_parse[begin][end][best_tag]
        if len(parse) == 1:
            return Tree(best_tag, [self.build_tree_recursive(begin, end, sentence, parse[0])])
        else:
            return Tree(best_tag, [self.build_tree_recursive(begin, parse[0], sentence, parse[1]), self.build_tree_recursive(parse[0], end, sentence, parse[2])])


    def build_tree_recursive(self, begin, end, sentence, best_tag):
        #print (begin, end, best_tag)
        #print (sentence)
        #print (self.background_parse[begin][end][best_tag])
        if end - begin == 1:
            if self.background_parse[begin][end][best_tag] not in sentence:
                return Tree(best_tag, [self.build_tree_recursive(begin, end, sentence, self.background_parse[begin][end][best_tag][0])])
            else:
                return Tree(best_tag, [Tree(self.background_parse[begin][end][best_tag])])
        else:
            best_parse = self.background_parse[begin][end][best_tag]
            #print (len(best_parse))
            if len(best_parse) == 1:
                return Tree(best_tag, [self.build_tree_recursive(begin, end, sentence, self.background_parse[begin][end][best_tag][0])])
            else:
...
class Lexicon:
    ...
    def __init__(self, train_trees):
        ...
        for train_tree in train_trees:
            ...
            #added by Yichen
            train_tree = TreeAnnotations.annotate_tree(train_tree)
...
if __name__ == '__main__':
    ...
    #opt_parser.add_option("--data", dest="data", default = "miniTest")
