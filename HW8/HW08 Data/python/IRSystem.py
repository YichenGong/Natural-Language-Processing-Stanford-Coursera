...
class IRSystem:
        ...
        for word in self.vocab:
            for d in range(len(self.docs)):
                ...
                if d in self.inv_index[word]:
                    self.tfidf[word][d] = ( 1 + math.log(float(self.inv_index[word][d]), 10)) * math.log((float(len(self.docs)) / len(self.inv_index[word])), 10)
                else:
                    self.tfidf[word][d] = 0
    ...
    def index(self):
        ...
        for index, doc in enumerate(self.docs):
            for word in doc:
                if index in inv_index[word]:
                    inv_index[word][index] += 1
                else:
                    inv_index[word][index] = 1
    ...
    def get_posting(self, word):
        ...
        # index_count = sorted( self.inv_index[word].items(), key=operator.itemgetter(1), reverse=True)
        # posting = [i for i,c in index_count]
    ...
    def boolean_retrieve(self, query):
        ...
        common_doc = set(self.get_posting(query[0]))
        for word in query:
            common_doc = common_doc & set(self.get_posting(word))

        # ------------------------------------------------------------------

        return list(common_doc)   # sorted doesn't actually matter
