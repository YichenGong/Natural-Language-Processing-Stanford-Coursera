...
class NaiveBayes:
  ...
  def classify(self, words):
    ...
    for token in wordsSet:
      ...
      negScore -= math.log(self.negWordCount + negVocab + posVocab)
  ...
  def addExample(self, klass, words):
    ...
    if (klass == "pos"):
      ...
      for word in words:
        if word in self.posWordSet:
          self.posFeatureCount[word] = self.posFeatureCount[word] + 1
        self.posWordCount += 1
    else:
      self.negCommentCount += 1
      for word in words:
        if word in self.negWordSet:
          self.negFeatureCount[word] = self.negFeatureCount[word] + 1
        self.negWordCount += 1
  ...
  def loadSentimentLexicon(self):
    fh = open("../data/subjclueslen1-HLTEMNLP05.tff", "r")
    line = fh.readline()
    while (line != ""):
      lineSplit = line.rstrip().split(" ")
      if (len(lineSplit) == 6):
        if(lineSplit[5].split("=")[1] == "negative"):
          self.negWordSet.add(lineSplit[2].split("=")[1])
        elif(lineSplit[5].split("=")[1] == "positive"):
          self.posWordSet.add(lineSplit[2].split("=")[1])
      line = fh.readline()
